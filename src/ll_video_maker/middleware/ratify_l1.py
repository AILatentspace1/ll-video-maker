...
def check_script(output_dir: str, duration: str = "1-3min") -> list[str]:
    ...
    for scene_block in re.split(r"(?=^## Scene)", text, flags=re.MULTILINE):
        match = re.search(r"^type:\s*(\w+)", scene_block, re.MULTILINE)
        if not match or match.group(1) not in audio_types:
            continue
        scene_id_match = re.search(r"^## Scene (\S+)", scene_block, re.MULTILINE)
        scene_id = scene_id_match.group(1) if scene_id_match else "?"
        for field_name in ("scene_intent:", "content_brief:", "narration:"):
            if field_name not in scene_block:
                errors.append(f"Scene {scene_id} 缺少 {field_name}")
    ...
    for block in re.findall(r"^## Scene.*?(?=^## Scene|\Z)", text, re.MULTILINE | re.DOTALL):
        if "type: data_card" not in block:
            continue
        scene_id_match = re.search(r"^## Scene (\S+)", block, re.MULTILINE)
        scene_id = scene_id_match.group(1) if scene_id_match else "?"
        if "data_semantic:" not in block:
            errors.append(f"Scene {scene_id} (data_card) 缺少 data_semantic")
        if re.search(r"items:\s*\[\]", block) or "items:" not in block:
            errors.append(f"Scene {scene_id} (data_card) items 为空")

    errors.extend(check_script_plan(output_dir))
    errors.extend(check_script_contract(output_dir))
    errors.extend(check_script_plan_consistency(output_dir))
    return errors
