...
def check_script_plan_consistency(output_dir: str) -> list[str]:
    ...
    if len(plan_scenes) != len(script_scenes):
        errors.append(f"script 与 plan 场景数不一致: script={len(script_scenes)}, plan={len(plan_scenes)}")
    ...
    for i in range(compare_len):
        ...
        if str(plan_scene.get("type", "")).strip() and script_scene["type"] != str(plan_scene.get("type", "")).strip():
            errors.append(f"Scene {script_scene['index']} type 偏离 plan: script={script_scene['type']}, plan={plan_scene.get('type')}")
        plan_topic = normalize_contract_topic(plan_scene.get("contract_topic", ""))
        if plan_topic and script_scene["contract_topic"] != plan_topic:
            errors.append(f"Scene {script_scene['index']} contract_topic 偏离 plan: script={script_scene['contract_topic'] or '缺失'}, plan={plan_topic}")
        plan_role = str(plan_scene.get("narrative_role", "")).strip()
        if plan_role and script_scene["narrative_role"] != plan_role:
            errors.append(f"Scene {script_scene['index']} narrative_role 偏离 plan: script={script_scene['narrative_role'] or '缺失'}, plan={plan_role}")
        ...
        if plan_duration > 0 and abs(script_duration - plan_duration) > max(1.5, plan_duration * 0.35):
            errors.append(f"Scene {script_scene['index']} duration 偏离 plan: script={script_duration}, plan={plan_duration}")
